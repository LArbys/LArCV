...
namespace larcv {
  ...
  size_t IOManager<T>::register_producer(const std::string& name)
  {
    ...
    _product_v[_product_ctr] = new std::vector<T>();
    _product_ctr+=1;
    ...
    if(_io_mode != kWRITE) {
      ...
      LARCV_DEBUG() << "Branch name: " << br_name << " data pointer: " << _product_v[id] << std::endl;
      auto in_tree_ptr = new TChain(tree_name.c_str(),tree_desc.c_str());
      in_tree_ptr->SetBranchAddress(br_name.c_str(), &(_product_v.back()));
    ...
    if(_io_mode != kREAD) {
      ...
      auto out_br_ptr = _out_tree_v[id]->Branch(br_name.c_str(), &(_product_v[id]));
