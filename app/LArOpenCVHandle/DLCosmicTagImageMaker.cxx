#ifndef DLCOSMICTAG_IMAGEMAKER_CXX
#define DLCOSMICTAG_IMAGEMAKER_CXX

#include "DLCosmicTagImageMaker.h"
#include "DataFormat/EventPixel2D.h"

namespace larcv {

  void
  DLCosmicTagImageMaker::Configure(const PSet& pset) {
  ...
  /*
  std::vector<cv::Mat>
  DLCosmicTagImageMaker::ExtractMat(const std::vector<Image2D>& image_v) {
  ...
  cv::Mat
  DLCosmicTagImageMaker::ExtractMat(const Image2D& image) {
  ...
  DLCosmicTagImageMaker::ExtractImage(const Image2D& image,size_t plane) {
    std::vector<Image2D> image_v(plane+1);
    image_v[plane] = image;
    auto img_data_v = ExtractImage(image_v);
    return img_data_v[plane];
  }


  std::vector<std::tuple<cv::Mat,larocv::ImageMeta> >
  DLCosmicTagImageMaker::ExtractImage(const std::vector<Image2D>& image_v) {
  ...
  void
  DLCosmicTagImageMaker::ConstructCosmicImage(IOManager& mgr,
					 std::string producer,
  ...
  Image2D
  DLCosmicTagImageMaker::ConstructCosmicImage(const EventPixel2D* ev_pixel2d,
