...
def apply_ll_vars(df,pdf_m,pid):
    ...
    for name, item in pdf_m.items():
        ...
        print
        print "@name=",name,"@func=",func,"@args=",args,"@pid=",pid
...
def define_LLem_vars_Y():
    ...
    key2 = "nueid_par1_triangle_baselength_U"
    bb_v = np.arange(0,20+0.5,0.5)
    func = pparam_plane_ratio
    args = ["XXX",key1,key2,2,]
    pdf_m[key0] = (bb_v,func,args)

    key0 = "emY06"
    key1 = "nueid_par1_polybranches_U_v"
    bb_v = np.arange(0,10+1,1)
    func = multi_pparam_v_plane
    args = ["XXX",key1,2,]
    pdf_m[key0] = (bb_v,func,args)
...
def generate_llem_pdfs(df1,df2,mcinfo1,mcinfo2):
    ...
    SS2_   = "(mcinfo_scedr<5 and parentPDG==14 and " + aNC + ")"
    ...
    df_sig = df_sig.join(df_sig_mcinfo,rsuffix="_mcinfo")
    df_bkg = df_bkg.join(df_bkg_mcinfo,rsuffix="_mcinfo")
    ...
    df_sig = df_sig.sort_values(['mcinfo_scedr'],ascending=True).groupby(RSE).head(1)
    df_bkg = df_bkg.sort_values(['mcinfo_scedr'],ascending=True).groupby(RSE).head(1)
    ...
    write_nue_pdfs(sig_spec_m, bkg_spec_m, "revised_LLem_pdfs.root")
    
    print "Done"


def generate_llem_Y_pdfs(df1,df2,mcinfo1,mcinfo2):
    ...
    print
    print "@sig sz=",df_sig.index.size
    print
    df_sig = apply_ll_vars(df_sig,pdf_m,"geid")

    print
    print "@bkg sz=",df_bkg.index.size
    print
    df_bkg = apply_ll_vars(df_bkg,pdf_m,"eid")
    
    sig_spec_m, bkg_spec_m = generate_ll_spec(df_sig,df_bkg,pdf_m)

    write_nue_pdfs(sig_spec_m, bkg_spec_m, "revised_LLem_Y_pdfs.root")
...
def generate_llpc_Y_pdfs(df1,df2,mcinfo1):
    ...
    write_nue_pdfs(sig_spec_m, bkg_spec_m, "revised_LLpc_Y_pdfs.root")
...
def apply_ll_y(COMB_DF, LLEP, LLPC):
    ...
    pdf_em_m = define_LLem_vars_Y()
    pdf_pc_m = define_LLpc_vars_Y()
    
    precut = "((c01==1)and(c02==1)and(c34==1)and(c36==1)and(c37==1)and(c44==1)and(c45==1))"

    out_df['passed_ll_precuts_y'] = int(0)
    df_precut = out_df.query(precut).copy()
    df_rest   = out_df.drop(df_precut.index).copy()
    ...
    if df_precut.empty == True:
        ...
        df_precut['passed_ll_precuts_y'] = int(1)
...
def select_ll_y(in_df):
    out_df = in_df.copy()

    out_df['LLemY_LL'] = np.nan
    out_df['LLpcY_LL'] = np.nan

    nstep = int(10)
    for i in xrange(0,nstep):
        SS1_ = 'LLemY_pass%02d' % i
        SS2_ = 'LLpcY_pass%02d' % i
        out_df[SS1_] = 0
        out_df[SS2_] = 0

    out_df['ll_selected_y'] = 0

    pass_df = out_df.query("passed_ll_precuts_y==1").copy()
    fail_df = out_df.drop(pass_df.index).copy()

    if pass_df.empty == True:
        return out_df

    pass_df['LLemY_LL'] = pass_df.apply(LLemY_LL,axis=1)
    pass_df['LLpcY_LL'] = pass_df.apply(LLpcY_LL,axis=1)

    pass_df['LLemY_pass_v'] = pass_df.apply(LLemY_cut,args=(nstep,),axis=1)
    pass_df['LLpcY_pass_v'] = pass_df.apply(LLpcY_cut,args=(nstep,),axis=1)

    for i in xrange(0,nstep):
        SS1_ = 'LLemY_pass%02d' % i
        SS2_ = 'LLpcY_pass%02d' % i
        pass_df[SS1_] = pass_df['LLemY_pass_v'].str[i].values
        pass_df[SS2_] = pass_df['LLpcY_pass_v'].str[i].values

    pass_df.drop(["LLemY_pass_v"],axis=1,inplace=True)
    pass_df.drop(["LLpcY_pass_v"],axis=1,inplace=True)

    ll_index = pass_df.sort_values(['LLemY_LL'],ascending=False).groupby(RSE).head(1).index

    out_df = pd.concat([pass_df,fail_df],ignore_index=True)
    
    out_df.loc[ll_index, 'll_selected_y'] = 1
...
LLemY_v = ["LLemY%02d" % i for i in xrange(12)]
def LLemY_LL(row):
    ret = np.nan
    
    if row['LLemY00'] != row['LLemY00']:
        return ret

    data_v = row[LLemY_v].astype(np.float32)
    ret = float(data_v.sum())
    
    return ret

LLpcY_v = ["LLpcY%02d" % i for i in xrange(8)]
def LLpcY_LL(row):
    ret = np.nan
    
    if row['LLpcY00'] != row['LLpcY00']:
        return ret

    data_v = row[LLpcY_v].astype(np.float32)
    ret = float(data_v.sum())
