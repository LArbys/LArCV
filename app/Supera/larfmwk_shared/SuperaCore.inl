...
template <class S, class T, class U, class V, class W>
void SuperaCore<S, T, U, V, W>::set_chstatus(::larcv::PlaneID_t plane, unsigned int wire, short status)
{
    ...
    const std::vector<W>& simch_v)
{
  ...
  for (size_t p = 0; p < ::larcv::supera::Nplanes(); ++p) {
    ...
    std::cout << "Creating Event image frame:" << std::endl;
  ...
  for (size_t p = 0; p < ::larcv::supera::Nplanes(); ++p) {
    ...
    ::larcv::Image2D img(std::move(comp_meta),
                         std::move(_full_image.copy_compress(
                                     _event_image_rows[p],
                                     _event_image_cols[p])));
    event_image_v->Emplace(std::move(img));
  }
...
template<class S, class T, class U, class V, class W>
larcv::ImageMeta SuperaCore<S, T, U, V, W>::format_meta(const larcv::ImageMeta& part_image,
