...
template <class T, class U, class V, class W>
void MCParticleTree<T, U, V, W>::RegisterSecondary(
...
template <class T, class U, class V, class W>
void MCParticleTree<T, U, V, W>::RegisterSecondary(
  const larcv::Vertex& vtx,
  const larcv::ROI& secondary)
{
  LARCV_DEBUG() << "start" << std::endl;
  if (_roi_m.empty()) {
  ...
  if (iter != _roi_m.end()) (*iter).second.second.push_back(secondary);
    ...
    for (auto const& key_value : _roi_m)
      time_order.emplace(std::make_pair(key_value.first.T(),
                                        key_value.first));
