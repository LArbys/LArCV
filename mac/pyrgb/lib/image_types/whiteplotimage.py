...
class WhitePlotImage(object):
    ...
    def set_plot_mat(self,imin,imax):
        ...
        #self.plot_mat  = self.plot_mat / np.max(self.plot_mat)
        
        # maxout the pixels so the background is white
        # normalize everything to one
        
        mm = np.max(self.plot_mat)
        ...
        #green
        GG = self.plot_mat[:,:,1]
        GG = np.where(GG >= R, GG, 0.0)
        self.plot_mat[:,:,1] = np.where(GG < G, GG, 0.0)
